...
const QuestionPaperBuilderPage = () => {
  ...
  const fetchCourses = async () => {
    try {
      ...
      if (!token) return;
      const response = await axios.get("http://localhost:5000/api/v2/academic/courses", {
  ...
  const fetchSavedPapers = async () => {
    try {
        const token = localStorage.getItem("token");
        const response = await axios.get("http://localhost:5000/api/v2/question-paper/list", {
            headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const generateWithAI = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:5000/api/v2/question-paper/llm/generate",
        payload,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.post(
        `http://localhost:5000/api/v2/question-paper${endpoint}`,
        payload,
        { headers: { Authorization: `Bearer ${token}` } }
      );
                        ...
                        <div key={paper._id} className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-6 hover:border-indigo-500/50 transition-all group relative overflow-hidden">
                            ...
                            <div className="flex justify-between items-start mb-4">
                                <span className={`px-2 py-1 rounded text-xs font-bold uppercase tracking-wider ${
                                    paper.mode === 'LLM' ? 'bg-fuchsia-500/20 text-fuchsia-400' : 'bg-indigo-500/20 text-indigo-400'
                                }`}>
                ...
                <div className="space-y-6">
                    <div className="flex justify-between items-center border-b border-slate-700 pb-4">
                        ...
                        <div className="flex items-center gap-4">
                            <div className={`flex items-center gap-2 text-sm font-medium px-4 py-1.5 rounded-full border ${
